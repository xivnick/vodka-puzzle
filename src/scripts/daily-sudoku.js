...
let data,state,selected=0,notesMode=false,key,submitting=false,loading=false,closed=false,rollover,completedAnswer=null,rejectedAnswer=null,ready=false,reinitPending=false,stateDirty=false;
...
function change(n){if(!ready||!state||data.givens[selected]!=='0')return;if(notesMode&&n){const a=state.notes[selected];state.notes[selected]=a.includes(n)?a.filter(v=>v!==n):[...a,n];}else{state.values[selected]=n;state.notes[selected]=[];}stateDirty=true;persist();render();if(state.values.every(Boolean)&&!notesMode)submit();}
...
function showPuzzle(puzzle, preserve=false){
 ...
 if(!preserve){
  ...
  state=clean(saved);stateDirty=false;completedAnswer=null;rejectedAnswer=null;
 }
 render();
}
...
async function init(){
 ...
 await window.puzzleAuthReady;
 if(cached&&window.puzzleAccount.user&&key===`daily-sudoku:guest:${cached.day}`){
  if(stateDirty){key=`daily-sudoku:${window.puzzleAccount.user.id}:${cached.day}`;persist();}
  else showPuzzle(cached);
 }
...
$('sudokuNotes').addEventListener('click',()=>{notesMode=!notesMode;$('sudokuNotes').setAttribute('aria-pressed',String(notesMode));});$('sudokuErase').addEventListener('click',()=>change(0));$('sudokuReset').addEventListener('click',()=>{if(confirm('입력한 숫자와 메모를 초기화할까요?')){state=clean(null);stateDirty=true;persist();render();}});
