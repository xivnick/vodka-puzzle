...
let data,state,selected=0,notesMode=false,key,submitting=false,loading=false,closed=false,rollover,completedAnswer=null,rejectedAnswer=null,ready=false;
...
function showPuzzle(puzzle, preserve=false){
 data=puzzle;document.querySelector('h1').textContent=title(data.day);document.title=`${title(data.day)} · vodka puzzle`;
...
function showUnavailable(text){
 setReady(false);state=null;data=null;
...
async function init(){
 ...
 try{
  ...
  clearTimeout(rollover);rollover=setTimeout(()=>{if(!day)init();else if(data?.available)refreshRanks().catch(()=>{});else init();},Math.max(1000,new Date(result.next_opens_at)-new Date(result.server_now)+100));
