...
let current,offset=0,busy=false,rollover;
...
function apply(data){
 ...
 updateCard(data.rankings);document.getElementById('rankTitle').dataset.dailyCount=String(data.rankings.length);updateRankTitle();
...
window.addEventListener('puzzle-auth-ready',refresh);
