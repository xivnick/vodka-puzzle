...
async function run({search='',profile=null,error=null,loggedIn=true}={}){
 ...
 const context=vm.createContext({createClient:()=>client,URL,URLSearchParams,Event,setTimeout,
  location:{origin:'https://puzzle.xivnick.me',pathname:'/auth/callback/',search,replace:p=>redirects.push(p)},
  history:{replaceState(){}},sessionStorage:{getItem:()=>'/practice/',removeItem(){}},
...
test('OAuth code is exchanged once and first login goes to nickname setup',async()=>{
 const r=await run({search:'?code=one-use-code'});assert.deepEqual(r.exchanges,['one-use-code']);assert.deepEqual(r.redirects,['/nickname/']);
});
test('existing account returns to its original puzzle',async()=>{
 const r=await run({search:'?code=code',profile:{nickname:'solver'}});assert.deepEqual(r.redirects,['/practice/']);
});
...
test('return path cannot redirect outside this site or back into auth',async()=>{
 ...
 for(const url of ['https://evil.example/','//evil.example/','javascript:alert(1)','/auth/callback/','/nickname/']){
  assert.equal(vm.runInContext(`safeNext(${JSON.stringify(url)})`,r.context),'/');
 }
