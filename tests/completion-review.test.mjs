...
test('review accepts a rule-compliant daily board and rejects conflicts, changed givens and malformed snapshots',()=>{
 ...
 const cases=[row,{...row,state:{values:[...values.slice(0,80),values[79]]}},{...row,givens:'9'+'0'.repeat(80)},{...row,state:null},{...row,state_version:2},{...row,puzzle_id:'260917_01',state:{values:'1'.repeat(81)}},{...row,puzzle_id:'260916_01',state:{version:1,rects:[{r0:0,c0:0,r1:8,c1:8}]}},{...row,puzzle_id:'260921_01',state:{values:equalSumValues}},{...row,puzzle_id:'unknown'}];
 ...
 assert.deepEqual(result.stdout.trim().split('\n').map(line=>JSON.parse(line).result),['valid','invalid','invalid','state_missing','unsupported_version','invalid','invalid','valid','unsupported_puzzle']);
});
