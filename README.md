...
- `Skyscraper.astro`: 가로·세로 라틴방진과 바깥 시야 단서를 검사하는 스카이스크레이퍼 화면. `/test/skyscraper/260920/`에서 사진 기반 6×6 문제를 기록·저장 없이 시연한다.
