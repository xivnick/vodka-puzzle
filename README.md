...
- 보름달 스도쿠: `/test/moon-sudoku/260925/`에서 숫자 힌트가 원 모양으로 놓인 정통 9×9 스도쿠를 시연한다. `src/lib/moon-sudoku.js`가 단서와 입력 규칙을 관리하며 진행 저장·완료 기록·공개 목록 등록은 하지 않는다. 테스트 랭킹은 토끼 아이콘을 붙인 가상 기록이며 실제 완료 데이터와 연결하지 않는다.
