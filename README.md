...
- `/terms/`, `/privacy/`는 Astro 페이지로 관리한다. 홈 하단 및 로그인/마이페이지에서 연결한다.
- `supabase/functions/delete-account/index.js`는 요청의 인증 토큰을 Auth 서버에서 검증하고 그 계정만 영구 삭제한다. 관리자 키는 Supabase 함수 환경에서만 사용한다.
- 함수의 `verify_jwt`는 false다. 새 서명 방식과 관계없이 함수 내부의 `/auth/v1/user` 검증을 거쳐야 삭제가 가능하다.
- `20260917_account_deletion.sql`은 인증 계정 삭제 시 학기별 닉네임·완료·진행 기록이 함께 삭제되도록 외래 키를 변경한 적용 완료 기록이다.
- 정적 아카이브와 별도 로컬 백업은 계정 삭제로 수정되지 않는다. 삭제 요청 시 원본 스냅샷·게시 파일·백업을 함께 확인해야 한다. 백업 복원 시 삭제한 계정 정보를 재게시하지 않는다.
