...
- 한가위 스도쿠: `/260925_01/`에서 숫자 힌트가 원 모양으로 놓인 정통 9×9 스도쿠를 제공한다. `src/lib/moon-sudoku.js`가 단서와 입력 규칙을 관리하며 로컬·클라우드 진행 저장과 완료 기록을 지원한다. 완성하면 원 영역이 짧은 전환과 함께 옅은 노란색으로 채워지고 `풍성한 한가위 되세요!`를 표시한다. 한가위 연휴인 2026-09-24~27에는 이 문제를 완성한 사용자의 전체 랭킹 이름 오른쪽에 토끼 아이콘을 표시한다. `/test/moon-sudoku/260925/`는 저장과 기록을 분리한 시연 경로이며, `?completed=1`은 완성 예시, `?completion-test=1`은 가운데 한 칸만 비운 완성 테스트다.
