...
- 한가위 스도쿠: `/260925_01/`에서 숫자 힌트가 원 모양으로 놓인 정통 9×9 스도쿠를 제공한다. `src/lib/moon-sudoku.js`가 단서와 입력 규칙을 관리하며 로컬·클라우드 진행 저장과 완료 기록을 지원한다. 완성하면 원 영역이 짧은 전환과 함께 옅은 노란색으로 채워지고 `풍성한 한가위 되세요!`를 표시한다. 한가위 연휴인 2026-09-24~27에는 이 문제를 완성한 사용자의 전체 랭킹, 데일리 스도쿠 랭킹, 각 문제의 `푼 사람` 이름 오른쪽에 토끼 아이콘을 표시한다. `/test/moon-sudoku/260925/`는 저장과 기록을 분리한 시연 경로이며, `?completed=1`은 완성 예시, `?completion-test=1`은 가운데 한 칸만 비운 완성 테스트다.
...
새 문제는 **별도 테스트 URL에서 시연 → 사용자 피드백에 따른 수정 → 명시적인 공개 승인 → 정식 문제 경로와 목록에 반영** 순서로 진행한다. 승인 전에는 공개 목록에 등록하지 않으며 실제 완료 기록·클라우드 진행 저장을 연결하지 않는다. 테스트 URL 자체는 인증된 비공개 경로가 아니다. 사진 기반 온도계 두 문제는 `/test/thermo-sudoku/photo-20260918-1/`(9×9)과 `/test/thermo-sudoku/photo-20260918-2/`(6×6)에서, 사진 기반 사풍 두 문제는 `/test/four-winds/1/`과 `/test/four-winds/2/`에서, 사진 기반 스카이스크레이퍼 문제는 `/test/skyscraper/260920/`에서, 사진 기반 같은 합 스도쿠는 `/test/equal-sum-sudoku/260921/`에서, 260922 수식완성 1·2·3은 `/test/formula-completion/260922/`·`/test/formula-completion/260922-2/`·`/test/formula-completion/260922-3/`에서 시연하며 기록·진행 저장을 하지 않는다. 공개된 사풍 문제는 `/260919_01/`과 `/260919_02/`, 스카이스크레이퍼 문제는 `/260920_01/`, 같은 합 스도쿠는 `/260921_01/`, 수식완성 세 문제는 `/260922_01/`·`/260922_02/`·`/260922_03/`에서 서비스한다.
